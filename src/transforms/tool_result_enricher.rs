...
/// Detects Claude ToolResult nodes whose text carries structured `toolUseResult` JSON and
/// attaches a [`ToolResultUiState`] so they render as rich widgets.
///
/// Only registered for the Claude provider.
...
impl ToolResultEnricher {
    pub fn new(config: &FileDeltaWidgetConfig, workspace_path: Option<std::path::PathBuf>) -> Self {
        Self {
