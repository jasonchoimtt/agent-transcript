...
impl App {
    pub(super) fn draw(&mut self, tui: &mut DefaultTerminal) -> color_eyre::Result<Option<Rect>> {
        ...
        tui.draw(|frame| {
            ...
            // Cursor: place inside the prompt overlay if the cursor is in overlay rows.
            if terminal_active
                && let Some((area_x, overlay_y, prompt_h, pbsr)) =
                ...
                && let PanelState::Live { ts: ref term, .. } = self.terminal.state
            {
                ...
                if crow >= pbsr && !screen.hide_cursor() {
                    ...
                    if row_in_overlay < prompt_h {
                        frame.set_cursor_position((area_x + 1 + ccol, overlay_y + row_in_overlay));
                    }
                }
            }
            ...
            // When the overlay is active, suppress rows >= prompt_box_start_row (drawn there instead).
