...
## [0.1.2](https://github.com/jasonchoimtt/agent-transcript/compare/v0.1.1...v0.1.2) - 2026-09-24

### Added

- *(cursor)* link background-task notifications to their tool calls
- summarize and dim background-task notifications
- pass args after `--` through to the agent CLI
- toggle light/dark theme at runtime with Shift-T
...
- fall back to command when Bash/Shell tool description is absent
- prevent nested sub-agent sessions from hijacking followed transcript
- add Ctrl-Z/Ctrl-Y suspend/resume for the embedded agent process
- shift-tab not working
- *(cursor)* hide subagent sessions from the transcript picker
- *(cursor)* read Task result details from the tool message
...
- fix orphaned child processes leaked by suspend/resume panel tests
- fix defaults test failure
